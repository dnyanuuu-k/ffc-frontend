...
class CountryInput extends Component {
	...
	showPicker = () => {
		const visibleOptions = this.state.countries;
	...
	closePicker = (event) => {
		...
		if (!countryPicker?.contains(event.relatedTarget)) {
			...
			this.setState(
				{
					pickerVisible: false,
					visibleText: hasSelected ? visibleText : undefined,
				}				
			);
		}
	};
	...
	handleChangeText = (visibleText) => {
		...
		const visibleOptions = this.state.countries?.filter(({ name }) =>
	...
	render() {
		...
		const { pickerVisible, visibleText, countryMap } = this.state;
		const borderColor = pickerVisible
			? colors.primaryBlue
			: colors.borderColor;
		const label =
			visibleText === undefined
				? countryMap[props.value]
				: visibleText;
		return (
			<>
				<View
		...
		return visibleOptions.map((option, idx) => {
			...
			return (
				<TouchableOpacity
					...
					key={option.value}
				>
					<Text style={[style.optionText, { color }]}>
