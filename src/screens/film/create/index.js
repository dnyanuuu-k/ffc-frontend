...
class CreateFilm extends Component {
	static FILM_DETAILS = 1;
	static SUBMITTER_DETAILS = 2;
	static CREDIT_INFO = 3;
	static FILM_SPECS = 4;
	static FILM_AWARDS = 5;
	constructor(props) {
		super(props);
		this.state = {
			isBusy: false,
			currentTab: CreateFilm.CREDIT_INFO,
			tabs: tabList,
			formData: {},
	...
	componentDidMount() {
		const filmId = 1;
		this.initData(filmId);
	}

	initData = async (filmId) => {
		try {
			this.setState({ isBusy: false });
			const response = await Backend.Film.filmData(filmId);
			if (response.success) {
	...
	setFormData = (festival) => {
		const festivalDate = festival.festivalDates?.[0] || {};
		festivalDate.festivalDateId = festivalDate?.id;
		delete festivalDate.id;
	...
	renderForm = () => {
		...
		switch (currentTab) {
			case CreateFilm.FILM_DETAILS:
				return <FilmDetails {...props} />;
			case CreateFilm.SUBMITTER_DETAILS:
				return <SubmitterInfo {...props} />;
			case CreateFilm.CREDIT_INFO:
				return <CreditInfo {...props} />;
			case CreateFilm.FILM_SPECS:
				return <CategoryDetails {...props} />;
			case CreateFilm.FILM_AWARDS:
			...
			<>
				<Page headerComponent={<Header />} contentStyle={style.row}>
					<View style={style.sectionHolder}>
						<Text style={style.title}>Create New Film</Text>
						<Text style={style.subTitle}>
							In just five simple steps create you film page
						</Text>
...
export default withRouter(CreateFilm);